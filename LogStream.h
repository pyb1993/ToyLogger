...
#ifndef UNTITLED_LOGSTREAM_H
#define UNTITLED_LOGSTREAM_H
#include <assert.h>
#include <string.h>
#include <string>
#include <pthread.h>
...
namespace muduo
{
    ...
    namespace detail
    {
        const int kSmallBuffer = 4 * 1024; // 4kB
        const int kLargeBuffer = 1024 * 1024 * 4;// 4MB


        // FixedBuffer, 会被LogStream实例化而持有
        template<int SIZE>
        class FixedBuffer
        {
        public:
        ...
        class CircularBufferTemplate{
            ...
            char* peek(size_t& len) {
                // 这里只有wirte_count存在并发变化(增加)的可能, 因此read_count < write_count肯定是成立的
        ...
        private:
                ...
                void setZero_(size_t origin_start, size_t len){
                    ...
                    if(len >  overflow_len){
                        ...
                        auto real_len = read_count + capacity - origin_start;// len - overflow_len
            ...
            public:
                ...
                //std::atomic_llong idle_count insert_dummy();// 到目前为止还有多少,注意这里不能是无符号,否则会溢出
        ...
        public:
            explicit WriteBarrier(StateVec& states):states_(states)
            {
                ...
                for(auto& state: states){
                    ...
                    while(true){
                        old = state;
                        if((old & WRITE_MASK) == 0 && state.compare_exchange_strong(old, old | WRITE_MASK)){
                            break;
                        }
                    }
                }
    ...
    public:
        ...
        // 只有前面的都没有办法重载的情况下, 才会去执行const T& v
        ...
        self& operator<<(const muduo::string& v) // FIXME: StringPiece
        {
    ...
    public:
        explicit FmtMicroSeconds(int microSeconds) {
            pthread_once(&ponce_, &FmtMicroSeconds::init);   //第一次调用会在init函数内部创建，pthread_once保证该函数只被调用一次！！！！
